...
if [[ $OSTYPE == darwin* ]]; then
    ...
    alias update='echo "Upgrading packages..." && brew update && brew upgrade && brew cu && brew cleanup && brew cask cleanup && nvm use stable && npm update && npm update -g && sudo softwareupdate -i -a && gem update >/dev/null 2>&1 || echo "update Done!"'
else
    # Linux, use pacman
