...
if [[ $OSTYPE == darwin* ]]; then
    ...
    alias update='echo "Upgrading packages..." && cd ~ && sudo -v && brew update && brew upgrade && brew cu && brew cleanup && brew cask cleanup && nvm alias default lts/* && nvm use default && npm update && npm update -g && sudo softwareupdate -i -a && gem update >/dev/null 2>&1 || echo "update Done!"'
else
    # Linux, use pacman
