...
if [[ $OSTYPE == darwin* ]]; then
    ...
    alias update='echo "Upgrading packages..." && cd ~ && sudo -v && brew update && brew upgrade && brew cu -ayf && brew cleanup && brew cask cleanup && asdf plugin-update --all && sudo softwareupdate -i -a && gem update >/dev/null 2>&1 || echo "update Done!"'
else
    # Linux, use pacman
